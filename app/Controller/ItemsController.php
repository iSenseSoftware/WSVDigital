...
class ItemsController extends AppController {
	...
	public function repairExpDates() {
		// Access the live database and query the items table with exp dates
		$query = "SELECT DISTINCT Item.ItemCode, Item.TransLineItemUD6 as ExpDate from Items as Item where Item.Deleted = 0 ORDER BY Item.ItemCode asc";
		$liveItems = array ();
		$pdo = new PDO ( 'sqlsrv:Server=localhost\SQLEXPRESS;Database=' . __DBNAME__, 'mckenzie.jw', 'kollani' );
		foreach ( $pdo->query ( $query ) as $row ) {
			$liveItems [] = $row;
		}
		$this->set ( 'items', $liveItems );
		
		$localPDO = new PDO ( 'sqlsrv:Server=huswivc0237\SQLEXPRESS;Database=' . __DBNAME__, 'mckenzie.jw', 'kollani' );
		foreach ( $liveItems as $item ) {
			$localQuery = "Update Items set TransLineItemUD6 = '" . $item ['ExpDate'] . "' WHERE ItemCode = '" . $item ['ItemCode'] . "'";
			$localPDO->exec ( $localQuery );
		}
	}
	// public function rebuildHistory() {
	// // Go through history in active database after 09 Jan and redo the
	// // transactions in the local copy
	// $pdo = new PDO ( 'sqlsrv:Server=localhost\SQLEXPRESS;Database=' .
	// __DBNAME__, 'mckenzie.jw', 'kollani' );
	// // SELECT all history records that had been transferred
	// $activeQuery = "SELECT * from InventoryHistory where InventoryHistoryID >
	// 14323 and InventoryHistoryID < 14674 order by InventoryHistoryID asc";
	// $history = array ();
	// foreach ( $pdo->query ( $activeQuery ) as $row ) {
	// $history [] = $row;
	// }
	// $skips = array (
	// 3950,
	...
	// $historyType = $item ['InvHistoryTypeID'];
	// switch ($historyType) {
	// // Receive
	// case 1 :
	// // Do nothing! (yay)
	// $this->log ( "Receive, skip" );
	// break;
	// // Adjust
	// case 3 :
	// // Adjust the quantity
	// $itemID = $item ['ItemID'];
	// if ($itemID <= 4368 && ! in_array ( $itemID, $skips )) {
	// $query = "SELECT Quantity from Items where ItemID = " . $itemID;
	// $existing = $pdo->query ( $query );
	// $quantity = $existing->fetchColumn ( 0 );
	// $this->log ( 'Before Quantity: ' . $quantity );
	// $quantity = $quantity + $item ['QuantityChange'];
	// $this->log ( 'After Quantity: ' . $quantity );
	// $query = "Update Items Set Quantity = " . $quantity . " WHERE ItemID = "
	// . $itemID;
	// $pdo->query ( $query );
	// $this->log ( "Adjust " . $query );
	// }
	// break;
	// // Move From
	// case 4 :
	...
	// . $itemID;
	// $pdo->query ( $query );
	// $this->log ( "Move From " . $query );
	// }
	// break;
	// // Move To
	// case 5 :
	// // if in exception list, adjust quantity
	// $exceptions = array (
	// 14494,
	// 14583,
	// 14585,
	...
	// 14609
	// );
	// if (in_array ( $item ['InventoryHistoryID'], $exceptions )) {
	...
	// . $itemID;
	// $pdo->query ( $query );
	// $this->log ( "Issue " . $query );
	// }
	// break;
	// }
	// }
	// $this->set ( 'history', $history );
	// // Copy the history from the active database to the local copy
	// }
	...
	public function issue($id = null) {
		...
		if ($this->request->is ( 'get' )) {
			...
			// if the request is a POST, this indicates that data has been
			// submitted
			...
			if (is_array ( $this->request->data ['Item'] ['ItemID'] )) {
				...
				/*
				 * 1. Loop through the ItemID element of the array and populate
				 * an array representing a single item to be issued 2. Pass each
				 * array to a function which will perform a single item issue 3.
				 * If an error is encountered at any point, rollback the entire
				 * transaction
				 */
				...
				if ($item ['Item'] ['Quantity'] < 0) {
					...
					if ($this->Item->save ( $item )) {
						...
						// historyType defines the type of transaction to be
						// logged. 6 is issue
						$historyType = 6;
	...
	private function issueItem($input) {
		...
		if ($item ['Item'] ['Quantity'] < 0) {
			...
			if ($this->Item->save ( $item )) {
				...
				$inventoryHistory = array ();
				// historyType defines the type of transaction to be logged. 6
				// is issue
				$historyType = 6;
				$inv = $this->Item->find ( 'first', array (
						'conditions' => array (
	...
	public function receive() {
		...
		if ($this->request->is ( 'post' )) {
			...
			// TODO fix so that consumable items can be received to existing
			// location
			...
			if ($part ['Part'] ['ItemTypeID'] == 4) {
				...
				$existingItem = $this->Item->find ( 'first', array (
						'conditions' => array (
								'Item.ItemCode' => $part ['Part'] ['ModelCode'],
	...
	private function moveItem($input) {
		...
		if ($existingItem ['Item'] ['Quantity'] < 0) {
			...
			/*
			 * Determine if there is an existing entry for the given itemcode at
			 * the MoveTo location. 1. If there is an existing entry, adjust the
			 * quantities at the new and existing locations and save both
			 * records. 2. If there is no existing entry at that location,
			 * create a new Item record for the itemcode at the new location.
			 * Update both records. 3. Generate two Inventory History entries:
			 * MoveFrom and MoveTo
			 */
			$moveToHistoryType = 5;
	...
	/*
	 * The following functions are AJAX-listeners used for returning html chunks
	 * to the web client for dynamic DOM insertion
	 */
	public function ajaxCurrentStock($allowZero = false) {
		// I used the built-in PDO library for database access because using the
		// CakePHP
	...
	public function ajaxInvCtrls($partId = null) {
		if ($partId == null) {
			...
			// ),
			// 'fields' => array(
			// 'DISTINCT Item.ItemCode', 'Item.TransLineItemUD1',
			// 'Item.TransLineItemUD3'
			// ), 'contain' => array('Part' => array('fields' =>
			// array('Part.ModelName')))
			// ));
	...
	public function ajaxView($partId = null, $locationId = null, $allowZero = false) {
		if (isset ( $this->passedArgs ['allowzero'] )) {
			$allowZero = $this->passedArgs ['allowzero'];
		}
		if ($locationId == null) {
			if ($partId == null) {
				return null;
	...
	private function incrementInvControl() {
		...
		$incrementor = $this->BarcodeGenerator->find ( 'first', array (
				'fields' => array (
						'BarcodeGenerator.BarcodeGeneratorID',
						'BarcodeGenerator.IncrementingNumber',
						'BarcodeGenerator.Format' 
				) 
		) );
	...
	public function printSingleLabel($id = null, $qty = null) {
		if ($id === null || $qty === null) {
			...
			// Why the hell won't containable behavior work for
			// LocationTypes?!?!?!?!
			$item = $this->Item->find ( 'first', array (
					'conditions' => array (
							'Item.ItemID' => $id 
					...
					'fields' => array (
							'Item.ItemID',
							'Item.TransLineItemUD1',
							'Item.TransLineItemUD2',
							'Item.TransLineItemUD3',
							'Item.TransLineItemUD4',
							'Item.TransLineItemUD5',
							'Item.TransLineItemUD6',
							'Item.ItemCode' 
					),
					'contain' => array (
							'Location' => array (
			...
			if (file_exists ( "img/qr$itemCode.png" )) {
	...
	/*
	 * The mobile functions are, as the name suggests, for replicating the
	 * various CRUD functions for movile devices
	 */
	...
	/*
	 * the generateCert function generates a generic certificate of conformance
	 * for a given part in inventory
	 */
