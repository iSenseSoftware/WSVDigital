...
<? if (isset($item)) { ?>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#InventoryAdjustForm').submit(function(event){
        ...
        function populateParts(xml){
            ...
            $.ajax({
                type: "GET",
                url: "ajaxView/" + partId + '/allowzero:' + allowZero,
                dataType: "xml",
        ...
        function populateItems(xml){
            ...
            $('.addRow').click(function(){
                ...
                $('#adjustItemsTable tbody').append(added);
        ...
        function itemAdded(id){
            wasAdded = false
            $('#adjustItemsTable tr').each(function(){ 
        ...
        function validateForm(){
            formValidates = true
            if($('#adjustItemsTable tbody tr').length < 1){
                alert('No items selected to adjust');
                formValidates = false;
            }      
            $('#adjustItemsTable tbody tr').each(function(){
                errors = ''
                rowValidates = true
                adjustQuantity = $(this).find("[name='data[Item][Quantity][]']").val()
                ...
                if(adjustQuantity == ''){
                    ...
                    if(!isNumber(adjustQuantity)){
                        errors = errors + 'Must be a number'
                        rowValidates = false
                    }else{
                        adjustQuantity = parseFloat(adjustQuantity)
                        if(adjustQuantity < 0){
                            errors = errors + 'Qty must be positive or zero<br/>'
                            rowValidates = false
                        }
                    } 
                }
