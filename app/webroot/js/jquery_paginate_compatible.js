...
function joshPaginator(userOptions){
    ...
    // the variable 'me' is used to replace the usage of the prototype.bind() function
    // which is not compatible with ie8 or ff3
    var me = this;
    this.options = {
        base_url: 'fetchPage/',
    ...
    this.loadPage = function(html){
        var content_id = me.options.content_id;
        $(content_id).children().remove();
        $(content_id).append(html);
        $(me.options.results_id).val(me.options.result_count);
